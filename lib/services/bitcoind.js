...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  log.info('Using bitcoin config file:', spawnOptions.configPath);

  this.spawn = {};
  this.spawn.datadir = spawnOptions.datadir;
  this.spawn.exec = spawnOptions.exec;
  this.spawn.configPath = spawnOptions.configPath;
  this.spawn.config = {};
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    async.retry({times: 2000, interval: self.startRetryInterval}, function(done) {
      if (self.node.stopping) {
        exitShutdown = true;
        return done();
      }
